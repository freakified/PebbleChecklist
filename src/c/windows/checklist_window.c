...
// margin, (since anything else looks bad)
    ...
    graphics_draw_text(
        ctx, item->name, fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD),
        text_bounds, GTextOverflowModeTrailingEllipsis, alignment, s_text_att);
  }
  ...
  if (item->is_checked) {
    ...
    if (bounds.size.h == CHECKLIST_CELL_MIN_HEIGHT) {
      GSize text_size = graphics_text_layout_get_content_size(
          item->name, fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD), bounds,
...
static void draw_row_callback(GContext *ctx, Layer *cell_layer,
  ...
  layer_set_hidden(text_layer_get_layer(s_empty_msg_layer),
                   (checklist_get_num_items() != 0));
...
static int16_t get_cell_height_callback(struct MenuLayer *menu_layer,
  ...
  if (cell_index->row == 0 ||
      cell_index->row == checklist_get_num_items() + 1) {
    ...
    GSize size = graphics_text_layout_get_content_size_with_attributes(
        item->name, fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD),
        GRect(0, 0, width, 500), GTextOverflowModeTrailingEllipsis,
